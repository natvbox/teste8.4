...
export default function Groups() {
  ...
  const effectiveDraft = memberDraft.length ? new Set(memberDraft) : memberIds;

  const toggleMember = (id: number) => {
    setMemberDraft((prev) => {
      const base = prev.length ? new Set(prev) : new Set(memberIds);
      ...
      <div className="p-4 sm:p-8">
        ...
        <Dialog open={Boolean(membersGroup)} onOpenChange={(v) => (!v ? setMembersGroup(null) : null)}>
          <DialogContent className="sm:max-w-xl">
            <DialogHeader>
              <DialogTitle>Membros — {membersGroup?.name}</DialogTitle>
            </DialogHeader>
            ...
            <div className="mt-4 flex gap-2">
              <Button variant="secondary" className="w-full" onClick={() => setMembersGroup(null)}>
                Fechar
              </Button>
              <Button className="w-full" onClick={saveMembers} disabled={setMembersMutation.isPending}>
