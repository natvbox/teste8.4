...
// ✅ fonte única do nome do cookie (não usar COOKIE_NAME fixo aqui)
const SESSION_COOKIE_NAME = ENV.sessionCookieName || "app_session_id";
...
class SDKServer {
  ...
  async getUserInfo(accessToken: string): Promise<GetUserInfoResponse> {
    ...
    const loginMethod = this.deriveLoginMethod((data as any)?.platforms, (data as any)?.platform ?? (data as any)?.platform ?? null);
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // ✅ aqui é melhor NÃO criar usuário “do nada”
    let user = await db.getUserByOpenId(sessionUserId);

    if (!user) {
      // se quiser manter autocreate, aqui teria que validar openId.
      throw ForbiddenError("User not found");
    }

    // garante owner
    if (this.isSystemOwner(user.openId) && user.role !== "owner") {
      await db.upsertUser({ openId: user.openId, role: "owner", tenantId: null, lastSignedIn: signedInAt });
